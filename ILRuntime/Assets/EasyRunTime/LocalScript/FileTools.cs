...
{
    public static string netUrl = "https://raw.githubusercontent.com/likehuihui/HelpResouce/master";
  //  public static string netUrl = "file://D:/ServerResource";
    ...
    public static string GetLocalDLLPath(string dic = "")
    {
        ...
        return dir;
#else
        return string.Format("{0}/res/{1}/{2}", Application.persistentDataPath, GetPlatform, dic);
#endif 
    }
    public static string GetLocalPath(string direct,string name)
    {
#if UNITY_EDITOR
        DirectoryInfo temp = Directory.GetParent(Application.dataPath);
#else
        DirectoryInfo temp = Directory.GetParent(Application.persistentDataPath);
#endif

        string dir = string.Format("{0}/Res/{1}/{2}", temp.FullName, GetPlatform, direct);
        if (!Directory.Exists(dir))
        {
            Directory.CreateDirectory(dir);
        }
